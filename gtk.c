...
setup_work(int ind_lw, int ind_var)
{
	...
	for(int i = 10; i >= 0; i--) // хей костыль
			...
			case 6: {
				switch(ind_method) {
					case 0: { /* метод наискорейшего спуска */
						...
						struct xy xy1 = find_xy_min_cycle_descent(fs_lw7[ind_var], -10.0, -10.0,
						                                          low, high, eps);
						char output[100];
...
main(int argc, char *argv[])
{
  ...
  combo_lab_work = gtk_combo_box_new_text();
	...
	label_eps = gtk_label_new("\u03B5: 0.001");
	label_lambda = gtk_label_new("\u03BB: 0.1");
	label_low = gtk_label_new("low");
	label_high = gtk_label_new("high");
